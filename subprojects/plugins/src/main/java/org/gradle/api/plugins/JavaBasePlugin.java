...
public class JavaBasePlugin implements Plugin<ProjectInternal> {
    ...
    private <T> Provider<T> getToolchainTool(Project project, Transformer<Provider<T>, JavaToolchain> toolMapper) {
        final JavaPluginExtension extension = project.getExtensions().getByType(JavaPluginExtension.class);
        return getJavaToolchainQueryService().findMatchingToolchain(extension.getToolchain()).map(toolMapper).getOrElse(Providers.notDefined());
    }
